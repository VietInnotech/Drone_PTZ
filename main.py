...
def draw_overlay(frame, class_names, tracked_boxes, fps, proc_time, ptz, last_ptz_command, coverage, frame_index, detection):
    """Draw bounding boxes and informational overlay on the frame."""
    frame_h, frame_w = frame.shape[:2]
    detection_count = len(tracked_boxes)
    tracking_ids = [int(det.id) for det in tracked_boxes if hasattr(det, "id") and det.id is not None]
    ...
    for det in tracked_boxes:
        ...
        cv2.putText(frame, label_text, (x1, max(y1 - 10, 0)), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2, cv2.LINE_AA)
    ...
    for i, line in enumerate(detection_lines):
        cv2.putText(frame, line, (10, y0 + i*dy), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,255,0), 2, cv2.LINE_AA)
    for i, line in enumerate(ptz_lines):
        textsize = cv2.getTextSize(line, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2)[0]
        x = frame_w - textsize[0] - 10
        y = y0 + i*dy
        cv2.putText(frame, line, (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,128,0), 2, cv2.LINE_AA)
    y_bl = frame_h - dy * len(sys_lines) - 10
    for i, line in enumerate(sys_lines):
        cv2.putText(frame, line, (10, y_bl + i*dy), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,128,255), 2, cv2.LINE_AA)
...
def main():
    ...
    grabber_thread = threading.Thread(target=frame_grabber, args=(frame_queue, stop_event), daemon=True)
    ...
    try:
        while True:
            ...
            fps = len(fps_window) / (fps_window[-1] - fps_window[0] + 1e-6) if len(fps_window) > 1 else 0.0
            ...
            for det in tracked_boxes:
                cls_id = int(det.cls)
                conf = float(det.conf)
                if class_names[cls_id] == "drone" and conf > Config.CONFIDENCE_THRESHOLD:
                    if conf > best_conf:
                        best_det = det
                        best_conf = conf
            ...
            if best_det is not None:
                ...
                zoom_velocity = 0.0
                if abs(coverage_diff) > 0.03 and (now - last_zoom_time) >= Config.ZOOM_MIN_INTERVAL:
                    # Proportional ramping for zoom velocity
                    zoom_velocity = max(-1.0, min(1.0, coverage_diff * Config.ZOOM_VELOCITY_GAIN))
                ...
                if ptz.active:
                    ptz.stop()
                    last_ptz_command = "stop()"
                if zoom_active and (now - last_detection_time > Config.ZOOM_RESET_TIMEOUT):
                ...
                if not home_triggered and (now - last_detection_time) > Config.NO_DETECTION_HOME_TIMEOUT:
                    ptz.set_home_position()
                    last_ptz_command = "set_home_position()"
                    home_triggered = True

            draw_overlay(frame, class_names, tracked_boxes, fps, proc_time, ptz, last_ptz_command, coverage, frame_index, detection)
